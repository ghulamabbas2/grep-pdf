...
- [testing.md](docs/testing.md) — Testing strategy: pytest (`test_*.py`) for backend logic and Vitest (`*.test.ts`) for frontend utils, both living next to the file they test; Playwright MCP-driven E2E QA with no committed specs and unmocked API routes; a separate `.env.test` database; and independent, order-free tests. Read before writing tests or setting up QA.
- [deployment.md](docs/deployment.md) — Railway deployment via native GitHub integration (Railway builds/deploys, GitHub Actions only runs CI); PR Environments giving each PR an isolated ephemeral preview (own URL, Postgres, volume) auto-torn-down on close, with merge to `main` deploying prod; a single Dockerfile that builds the React frontend into the FastAPI image and runs uvicorn on Railway's assigned port; Alembic migrations on container start; and secrets in Railway env vars inherited from a base env. Read before touching deploy, CI, the Dockerfile, or environment config.
